...
import { Loader2, CheckCircle, X, Check, Clipboard, FileText, Type, Video, ChevronDown, Zap } from "lucide-react";
...
export default function App() {
  ...
  const generateAll = async () => {
    ...
    try {
      // Step 1: LinkedIn summaries (parallel)
      setGenProgress("Generating LinkedIn posts...");
      await Promise.all(arts.map(async (a, i) => {
        const txt = await callAPI([{ role: "user", content: `Summarize this article into a LinkedIn post using this EXACT format:\n\n[Short punchy headline based on the article]\n\n[1 sentence that supports the headline. Only add a second if truly necessary.]\n\nKey details:\n- [Key detail 1]\n- [Key detail 2]\n- [Key detail 3]\n\nRULES: No bold/markdown/hashtags/emojis/em-dashes. Plain text headline. Use - for bullets. Under 100 words. Professional tone. Blank lines between sections.\n\nAfter the post write ONLY the word VERIFIED on its own line if accurate.\n\nArticle:\n${a.content || a.description}` }]);
      ...
      // Step 2: Headline shortener (parallel)
      setGenProgress("Generating headline variations...");
      await Promise.all(arts.map(async (a, i) => {
        const txt = await callAPI([{ role: "user", content: `Shorten this headline into 3 punchy variations under 8 words each. Return ONLY JSON: {"headlines":[s,s,s],"visuals":{"images":[s,s],"icons":[s,s],"symbols":[s,s]}}. Headline: ${a.title}` }]);
        try {
          const p = JSON.parse(txt.replace(/```json|```/g, "").trim());
          newHlResults[i] = p.headlines || [];
          newHlVisuals[i] = p.visuals || null;
        } catch(e) { newHlResults[i] = []; }
      }));
      setHlResults([...newHlResults]);
      setHlVisuals([...newHlVisuals]);

      // Step 3: Jumpstart script
      setGenProgress("Generating Jumpstart script...");
      const scriptTxt = await callAPI([{ role: "user", content: `You are a broadcast scriptwriter for CBT News. Write a 2 to 2.5 minute daily news script called "The Jumpstart" using exactly the 4 articles provided.\n\nUse this EXACT format:\n\nGood morning. Welcome back to The Jumpstart on CBT News. Today is ${dayName}, ${dateStr}. Here are today's top stories, brought to you by Force Marketing.\n\nSTORY 1 – [ALL CAPS HEADLINE]\n[3-5 sentences. Each on its own line.]\n\nSTORY 2 – [ALL CAPS HEADLINE]\n[3-5 sentences.]\n\nSTORY 3 – [ALL CAPS HEADLINE]\n[3-5 sentences.]\n\nSTORY 4 – [ALL CAPS HEADLINE]\n[3-5 sentences.]\n\nOUTRO:\nThat's all for today's show but join me again tomorrow morning to get the top stories happening in automotive.\nAnd if you want more news and content coming to you throughout the day, be sure to follow us on all your favorite social media channels.\nHave an amazing ${dayName} and I'll see you back here tomorrow morning.\n\nARTICLE 1:\n${arts[0].content || arts[0].description}\n\nARTICLE 2:\n${arts[1].content || arts[1].description}\n\nARTICLE 3:\n${arts[2].content || arts[2].description}\n\nARTICLE 4:\n${arts[3].content || arts[3].description}` }], 2000);
  ...
  const generateCaptions = async () => {
    ...
    } catch { setCaptions(["Error generating captions."]); }
    ...
    <div style={{ background:"#f8fafc",minHeight:"100vh",fontFamily:"'Inter',system-ui,sans-serif",paddingBottom:48 }}>
      ...
      <div style={{ background:"#fff",borderBottom:"1.5px solid #e2e8f0",padding:"0 28px",display:"flex" }}>
        {tabs.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setTab(id)} style={{ display:"flex",alignItems:"center",gap:7,padding:"13px 18px",fontSize:12,fontWeight:tab===id?700:500,color:tab===id?TEAL:"#94a3b8",border:"none",background:"transparent",borderBottom:tab===id?"2px solid "+TEAL:"2px solid transparent",cursor:"pointer",marginBottom:-1.5,transition:"color .15s" }}>
            <Icon size={13} />{label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding:"24px 28px 0" }}>
          ...
          <div>
            ...
            {rssLoading ? (
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
                {[...Array(8)].map((_,i) => (
                  <div key={i} style={{ background:"#fff",border:"1.5px solid #e2e8f0",borderRadius:12,padding:"18px",height:100,animation:"pulse 1.5s ease-in-out infinite" }} />
                ))}
              </div>
            ) : (
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
                {rssArticles.map((a,i) => {
                  ...
                  return (
                    <button key={i} onClick={() => toggleSelect(i)} disabled={isDisabled}
                      ...
                      {isSelected && (
                        <div style={{ position:"absolute",top:10,right:10,width:22,height:22,borderRadius:"50%",background:ACCENTS[slotNum],display:"flex",alignItems:"center",justifyContent:"center" }}>
                          <span style={{ color:"#fff",fontSize:11,fontWeight:700 }}>{slotNum+1}</span>
                        </div>
                      )}
            ...
            {generated && (
              <div style={{ marginTop:16,background:"#f0fdf4",border:"1.5px solid #86efac",borderRadius:10,padding:"14px 18px",display:"flex",alignItems:"center",gap:10 }}>
                ...
                <span style={{ fontSize:13,fontWeight:600,color:"#166534" }}>All content generated! Switch to any tab to review and copy.</span>
              </div>
            )}
          </div>
        )}
        ...
        {tab === "linkedin" && (
          <div>
            {!generated && (
              <div style={{ background:"#fefce8",border:"1.5px solid #fde68a",borderRadius:10,padding:"14px 18px",marginBottom:16,fontSize:13,color:"#92400e" }}>
                Select 4 articles and click Generate All to populate this tab.
              </div>
            )}
                  ...
                  <div key={i} style={pn}>
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
                      <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                        <div style={{ width:20,height:20,borderRadius:"50%",background:ACCENTS[i],display:"flex",alignItems:"center",justifyContent:"center" }}>
                    ...
                    <div style={{ background:"#f8fafc",border:"1.5px solid #e2e8f0",borderRadius:8,padding:"12px 14px",fontSize:13,lineHeight:1.85,color:"#1e293b",fontFamily:"ui-monospace,monospace",minHeight:180,maxHeight:260,overflowY:"auto",whiteSpace:"pre-wrap" }}>
                      {sum ? (
                        <>{sum.trim()}{"\n\n———\n\nFind more here: "+articleLink}{sponsor && "\nToday's news presented by: "+(SPONSORS.find(x=>x.name===sponsor)||{}).name+" ("+(SPONSORS.find(x=>x.name===sponsor)||{}).url+")"}</>
                      ) : <span style={{ color:"#cbd5e1" }}>Will appear after generation...</span>}
                    </div>
                    {sum && (
                      <button onClick={() => doCopy(getPost(i), () => { setCopiedSummary(i); setTimeout(() => setCopiedSummary(null), 2000); })}
                        style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:6,width:"100%",marginTop:10,padding:"9px",fontSize:12,fontWeight:700,background:isCopied?"#16a34a":TEAL,color:"#fff",border:"none",borderRadius:7,cursor:"pointer" }}>
                        {isCopied ? <><Check size={12} />Copied!</> : <><Clipboard size={12} />Copy post</>}
                      </button>
                    )}
                  </div>
                );
                        ...
                        {results.map((h,j) => {
                          ...
                          return (
                            <div key={j} style={{ background:"#f8fafc",border:"1.5px solid #e2e8f0",borderLeft:"3px solid "+ACCENTS[i],borderRadius:8,padding:"12px 14px",display:"flex",flexDirection:"column",gap:8 }}>
                              ...
                              <button onClick={() => doCopy(h, () => { setCopiedHl(k); setTimeout(() => setCopiedHl(null), 2000); })}
                                style={{ padding:"5px 12px",fontSize:11,fontWeight:700,background:isCopied?"#16a34a":"transparent",color:isCopied?"#fff":TEAL,border:"1.5px solid "+(isCopied?"#16a34a":"#b2e0e4"),borderRadius:5,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:4 }}>
        ...
        {tab === "jumpstart" && (
          <div style={pn}>
            ...
            {jsScript && (
              <>
                <div style={dv} />
                <button onClick={() => doCopy(jsScript, () => { setJsCopied(true); setTimeout(() => setJsCopied(false), 2000); })}
                  style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:7,width:"100%",padding:"11px",fontSize:13,fontWeight:700,background:jsCopied?"#16a34a":TEAL,color:"#fff",border:"none",borderRadius:8,cursor:"pointer" }}>
                  {jsCopied ? <><Check size={13} />Copied!</> : <><Clipboard size={13} />Copy script</>}
                </button>
              </>
            )}
          </div>
        ...
        {tab === "caption" && (
          <div style={{ display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:16 }}>
            <div style={pn}>
              ...
              <div style={dv} />
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7 }}>
                <span style={sL}>Article context</span>
                <span style={{ fontSize:10,fontWeight:600,color:"#cbd5e1" }}>Optional</span>
              </div>
              <textarea value={context} onChange={e=>setContext(e.target.value)} placeholder="Paste the related article for added specificity..." style={{ ...inp,height:100,resize:"none" }} />
              <button onClick={generateCaptions} disabled={!transcript.trim()||capLoading}
                style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:7,width:"100%",marginTop:14,padding:"11px",fontSize:13,fontWeight:700,background:TEAL,color:"#fff",border:"none",borderRadius:8,cursor:(!transcript.trim()||capLoading)?"default":"pointer",opacity:(!transcript.trim()||capLoading)?0.4:1 }}>
              ...
              {captions.length > 0 ? (
                <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
                  {captions.map((c,i) => {
                    ...
                    return (
                      <div key={i} style={{ background:"#f8fafc",border:"1.5px solid #e2e8f0",borderLeft:"3px solid "+ACCENTS[i%3],borderRadius:8,padding:"13px 14px",display:"flex",gap:12,alignItems:"flex-start" }}>
                        <span style={{ flex:1,fontSize:13,color:"#1e293b",lineHeight:1.75,whiteSpace:"pre-wrap" }}>{c}</span>
                        <button onClick={() => doCopy(c, () => { setCapCopied(i); setTimeout(() => setCapCopied(null), 2000); })}
                          style={{ flexShrink:0,padding:"5px 12px",fontSize:11,fontWeight:700,background:isCopied?"#16a34a":"transparent",color:isCopied?"#fff":TEAL,border:"1.5px solid "+(isCopied?"#16a34a":"#b2e0e4"),borderRadius:6,cursor:"pointer" }}>
                          {isCopied ? <><Check size={11} />Copied</> : "Copy"}
                        </button>
                      </div>
                    );
                  })}
                ...
                <div style={{ background:"#f8fafc",border:"1.5px solid #e2e8f0",borderRadius:8,padding:"14px 16px",minHeight:200,fontSize:13,color:"#cbd5e1",fontFamily:"monospace" }}>
